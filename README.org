...
*** 获得动画实例
    ...
    想要更精细的控制动画的行为，首先要做的是在渲染动画的上下文中获取渲染后的动画实例对象，UiLottie 提供了一个 `afterCreate` 钩子函数，会在组件创建 lottie 的动画实例对象之后调用并且将这个实例对象传递给外部的上下文。以下是用法示例：
    ...
    #+BEGIN_SRC htmlbars
      <UiLottie @path="..." @afterCreate={{action this.afterCreate}} />
...
*** 播放/停止/暂停
    ...
    #+BEGIN_SRC htmlbars
      <UiLottie @path="..." @loop={{true}} @afterCreate={{action this.afterCreate}} />
